...
class StimulusGUI():
    ...
    def __init__(self) -> None:
        self.preset_file = os.path.abspath(
        ...
        self.stimulus_objects = [
            self.HMode, self.H, self.HMin, self.HSteps, self.HMax, self.HBack,
            self.HPhi, self.HPhiMin, self.HPhiSteps, self.HPhiMax,
            self.HPhiBack, self.HTheta, self.HThetaMin, self.HThetaSteps,
            self.HThetaMax, self.HThetaBack, self.fmin, self.fsteps, self.fmax,
            self.Idir, self.Vdir, self.Idc, self.Iac, self.LLGtime,
            self.LLGsteps
        ]
        ...
        for i in range(0, len(self.stimulus_objects)):
            self.stimulus_layout.addWidget(self.stimulus_objects[i].Label, 0,
                                           i)
            self.stimulus_layout.addWidget(self.stimulus_objects[i].Value, 1,
                                           i)
    ...
    def __dynamic_constructor(self, preset_file: str):
        ...
        for obj in preset_json["stimulus"]:
            setattr(self, obj["name"],
                    Labelled(**obj["params"], var_name=obj["name"]))
    ...
    def get_stimulus_object(self) -> StimulusObject:
        ...
        H_sweep, sweep = get_stimulus(self.H.Value.value() * 1e3,
                                      ...
                                      self.HPhiMax.Value.value(), steps, back,
                                      mode)
    ...
    def H_mode_changed(self):
        ...
        if mode == SweepMode.H:
            for i in [
                    self.HMin, self.HSteps, self.HMax, self.HBack, self.HPhi,
                    self.HTheta
            ]:
                i.show()
        ...
        if mode == SweepMode.PHI:
            for i in [
                    self.H, self.HPhiMin, self.HPhiSteps, self.HPhiMax,
                    self.HPhiBack, self.HTheta
            ]:
                i.show()

        if mode == SweepMode.THETA:
            for i in [
                    self.H, self.HPhi, self.HThetaMin, self.HThetaSteps,
                    self.HThetaMax, self.HThetaBack
            ]:
                i.show()
