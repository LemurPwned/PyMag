...
def get_stimulus2(Hmin, Hmax, ThetaMin, ThetaMax, PhiMin, PhiMax, STEPS, back, mode):
    ...
    if mode == "H":
        ...
        Hmag = (Hx**2+Hy**2+Hz**2)**0.5*np.sign( np.cos(np.arctan2(Hx,Hy))  )
...
def cos_between_arrays(vec1, vec2):
    return np.dot(np.array(vec1), np.array(vec2)) / (norm(np.array(vec1) * norm(np.array(vec2))))
